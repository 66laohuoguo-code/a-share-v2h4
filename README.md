...
可在 `data/input/accounts/<account_id>/fees.json` 配置该账户的实际券商费用：包含
`account_id`、数值型 `broker_commission_rate` 和 `broker_minimum_commission` 三个字段。
费率使用小数（例如万一为`0.0001`），最低佣金单位为元。周调仓在资金档选择后加载，
不因资金跨档改变该账户费率；没有该文件时使用策略原配置。也可向Python入口传入
`--account-fees`指定文件，文件内账户必须匹配。该覆盖不修改法定税费、旧周报或旧回测，
个人账户文件不应提交到公开仓库。
...
V2H4 核心分数由低 Beta、低波动、低换手、短期反转、行业趋势和较小回撤六个风格分量加权而成。具体权重属于按账户档位校准的部署参数，不随本仓库发布：程序启动时从被忽略的 `config/component_weights.local.json` 读取，未提供该文件时回退为等权分组（见 `component_weights.py`）。因此同一份代码在不同部署下的排名并不相同；要复现某一份历史结果，必须同时拿到那一份权重配置。
...
先单独建立共享因子快照。因子计算只执行一次，后续全部资金和持股数实验读取同一个缓存。当前版本还会把市场状态写入同一缓存数据库：
...
两个分片只读同一个行情库和共享因子缓存，但写入完全不同的输出目录。缓存完整时，回测不再把 `stock_daily` 全表常驻内存，而是按交易日从 SQLite 流式读取成交字段；市场状态和开盘缺口历史也会在第一次运行后复用。启动日志中的 `Price access mode: indexed SQLite daily streaming` 表示轻量路径已经生效。第一次使用新的日期区间或市场参数时仍需建立辅助缓存，第二次启动会更快。16GB 内存电脑仍建议最多使用两个并发进程；如果两个进程会影响日常使用，应改回 `ShardCount 1` 顺序运行。
